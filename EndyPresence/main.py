...
try:
  ...
  os.system(f'pip install -r {os.path.join(current_dir, "requirements.txt")}')
  if sys.platform == 'win32': ctypes.windll.user32.MessageBoxW(0, "Dependencies installed. Reopen the program to continue!", "EndyPresence", 0)
  elif sys.platform == 'darwin': os.system("osascript -e 'Tell application \"System Events\" to display dialog \"Dependencies installed. Reopen the program to continue!\" with title \"EndyPresence\"'")
  exit(0)
...
def update_rpc(limg, simg, ltxt, stxt, state, details, b1t, b1u, b2t, b2u):
  buttons = []
...
ui.rpcb.clicked.connect(lambda: update_rpc(str(ui.limge.currentText()), str(ui.simge.currentText()), ui.ltxte.toPlainText(), ui.stxte.toPlainText(), ui.statee.toPlainText(), ui.detailse.toPlainText(), ui.b1txte.toPlainText(), ui.b1urle.toPlainText(), ui.b2txte.toPlainText(), ui.b2urle.toPlainText()))
...
with open(os.path.join(current_dir, "Database.json"), "r+") as db:
  ...
  if firstrun["FR"] == 1:
    firstrunmsg = QMessageBox()
    firstrunmsg.setText('Check your browser!')
    firstrunmsg.exec()
...
try:
  ...
  update_rpc(str(ui.limge.currentText()), str(ui.simge.currentText()), ui.ltxte.toPlainText(), ui.stxte.toPlainText(),
             ui.statee.toPlainText(), ui.detailse.toPlainText(), ui.b1txte.toPlainText(), ui.b1urle.toPlainText(),
             ui.b2txte.toPlainText(), ui.b2urle.toPlainText())
except:
  rpcerror = QMessageBox()
  rpcerror.setText("Connection Error")
  rpcerror.setInformativeText("Can't connect to Discord RPC. Check if Discord is currently running.")
